...
const SurahListItem: React.FC<{ surah: Surah, onSelect: (surah: Surah) => void }> = ({ surah, onSelect }) => (
    ...
    >
      ...
      <div className="text-right">
        <p className="text-gray-600 dark:text-gray-300">{surah.numberOfAyahs} Verses</p>
      </div>
    </li>
);
...
const JuzListItem: React.FC<{ juz: Juz, onSelect: (juz: Juz) => void }> = ({ juz, onSelect }) => (
    ...
    >
      ...
      <div className="flex-grow">
        ...
        <p className="text-gray-500 dark:text-gray-400 text-sm">Part {juz.number}</p>
      </div>
    </li>
);
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="text-center mb-8">
        ...
        <h1 className="text-3xl font-bold" style={{ textShadow: '1px 1px 2px rgba(0,0,0,0.1)' }}>Al-Qur’an Self Recitation</h1>
        <p className="text-gray-500 dark:text-gray-400 mt-2">Choose a surah or juz to read or memorize.</p>
      </div>
      ...
      <div className="relative mb-8">
        <input
          type="text"
          placeholder={`Search for a ${listMode === 'surah' ? 'surah' : 'juz'}...`}
          value={searchTerm}
      ...
      {selectedJuz && (
           <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4" onClick={closeModal}>
                <div className="bg-gradient-to-b from-white to-gray-100 dark:from-dark-card dark:to-dark-surface rounded-2xl p-6 shadow-retro-xl text-center w-full max-w-xs border border-gray-300 dark:border-gray-700 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                    ...
                    <p className="text-gray-500 dark:text-gray-400 mb-6">Select a mode for Juz {selectedJuz.number}.</p>
                    <div className="space-y-4">
